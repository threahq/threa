...
A runtime session's identity is RECORDED, not derived: one JSON file per session under `~/.threa/harnessd/identities/` (override: `THREA_HARNESSD_IDENTITIES_DIR`), holding `runtimeSessionId`, `instanceId`, the canonical worktree, the `runtimeKind`, and a snapshot of the profile the directory was provisioned under. The legacy fallback — `sha256("<host>:<cwd>")` — changes with the DHCP-supplied hostname, which is why the store exists.

The key is **(directory, runtime kind)**, not directory alone. A Pi (`pi-local`) and a Claude (`claude-code-channel`) coexist in one cwd as two independent identities: spawn, adopt, revival, backfill, and profile lookup each resolve within their own kind's namespace, and a record with `runtimeKind: "unknown"` (pre-kind) counts as Claude everywhere. What stays per-directory is destruction — the reaper never commits, pushes, or removes a directory that another kind's record or live pane still claims (it winds such a link down under `preserve: "none"`), and a paneless live Claude vetoes removal whichever kind the record names. Within one kind, one directory holds at most one identity: a same-kind respawn supersedes the record, two same-kind records refuse loudly rather than mint a third.

Nothing about a record is permanent. The reaper retires it when the directory is actually removed, and deleting the file releases the binding by hand — the next spawn in that directory then mints a fresh identity. `threa-harnessd resolve [<ref>]` shows what each store currently attests.
...
- Claude sessions also get a second MCP server, `threa` (`bun packages/cli/src/cli.ts mcp serve`), so a session can read and write its workspace as its own bot. Its credentials live in a per-session CLI config at `~/.threa/harnessd/cli/<runtimeSessionId>.json` (mode 0600, `THREA_HARNESSD_MCP_DIR` moves the parent directory), written with `"principal": "bot"` so the CLI refuses to run if the key turns out to belong to a human. The same path is exported into the pane as `THREA_CONFIG`, which is what an attached thread's blanked `THREA_API_KEY` falls back to. A session with no bot credentials registers no `threa` server and warns on stderr. The `--mcp-config` definition wins over a same-named user-global registration (probed on Claude Code 2.1.273: only the session's `threa` command is spawned), and a channel name of `threa` is refused because it would share the key.
...
- Liveness has two independent sources: the tmux pane, and the Claude process table (`~/.claude/sessions/<pid>.json` corroborated against the start time in `/proc/<pid>/stat`, which is what Claude Code records as `procStart`). Either one saying "alive" refuses the launch. The pane check alone let nine consecutive passes each start another Claude in one worktree on 2026-07-28.
