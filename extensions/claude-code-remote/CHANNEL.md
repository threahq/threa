...
The extension is a single MCP server (`src/index.ts` → `ChannelServer`). Claude Code spawns it with `bun src/index.ts` when you start with `--dangerously-load-development-channels server:threa-channel`. Scratchpad linking is gated on that flag **matched against the registration's own key**: the MCP registration must carry `"env": {"THREA_CHANNEL_SERVER_KEY": "<registered server name>"}`, and `channel-detect.ts` links only when the parent Claude process's command line names that key in the channels flag. Without the env key, or with a flag naming a different registration, the server serves as a plain idle MCP (no scratchpad, no Threa traffic). Claude Code negotiates identical MCP client capabilities in channel and plain mode and a server cannot learn its own registration key from MCP, so the flag + registration-carried key together are the only reliable channel signal — one session can load several registrations of this same script with identical argv, and a constant-based check would activate all of them (the stale-duplicate twin then links the same scratchpad and silently eats invocations). This gate makes a global user-scope registration safe. Inside that one process:
...
1. Checks that its registration carries `THREA_CHANNEL_SERVER_KEY` and that the parent Claude process's command line names that key in `--dangerously-load-development-channels` (`channel-detect.ts`). If either fails — a plain Claude session, a registration without the env key, or a duplicate registration whose key isn't the flagged one — it stops here and idles; nothing below runs.
...
**Inbound (scratchpad → working directory).** The claim's hydrated history is text only — no attachment metadata, and the trigger message itself isn't in it. So before pushing the event, the channel lists the recent messages of the invocation's stream (`GET /streams/:id/messages`), picks the attachments on the source message plus any on the history Claude is being shown, downloads each into `.threa-attachments/<invocation_id>/<attachment_id>/` under the working directory (the per-id subdirectory keeps two files that share a name apart) (via a short-lived signed URL from `GET /attachments/:id/url`), and appends a manifest of local paths to the channel content. Claude reads the files straight from disk. The whole step is best-effort: a key without `attachments:read`, or any download failure, is logged and the prompt still goes through without the files.

**Outbound (working directory → scratchpad).** The `reply` tool's text may contain `THREA_ATTACH: <path>` lines. Before completing the invocation, the channel strips those lines, uploads each file (`POST /attachments`, multipart), and appends `[name](attachment:<id>)` links to the reply markdown. The backend associates the uploads with the posted message purely from those links — there is no separate attachment-ids field on the plaintext `complete`. A failed upload is reported inline rather than dropping the reply.

**Sealed (E2EE) scratchpads** vary both directions: inbound refs (per-file key, iv, real filename) arrive inside the sealed message payloads the channel decrypts, and the S3 ciphertext is fetched and decrypted locally; outbound files are encrypted under a fresh per-file key, uploaded as ciphertext with `e2e=true` (the server stores a placeholder row), the ref seals into the reply payload, and the ids bind via the `attachmentIds` field on `sealed-messages`/`sealed-complete` (the sealed body has no markdown links to parse).
