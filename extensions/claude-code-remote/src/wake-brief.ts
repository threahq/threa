...
export function formatWakeBrief(note: SessionWakeNote): WakeBrief | undefined {
  ...
  return {
    notice:
      ...
      "Your runtime process was replaced, so background shell jobs, timers, and shell state from before it are gone; " +
      "re-check anything you left running. A command still running counts as work and holds the sweep off by itself; " +
      `a wait of your own does not, so hold it off first with \`threa-harnessd hold ${note.runtimeSessionId} --minutes N\`. ` +
