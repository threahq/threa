 ...
 *
 * This extension ships standalone — it is copied to `~/.pi/agent/extensions/`
 * and `npm install`ed on the user's machine, where the private, unpublished
 * `@threa/crypto` workspace package cannot resolve. So the slice the sealed
 * (E2EE) bot path needs is copied here verbatim and depends only on the
 * published `@hpke/core` plus WebCrypto.
 *
...
// Pure-JS (noble) X25519 KEM, NOT `@hpke/core`'s native one. The harness runs in
// whatever runtime Pi uses; Bun's WebCrypto lacks X25519 `deriveBits`, so the
// native KEM throws `EncapError/DecapError: The algorithm is not supported` there.
// The noble KEM works in any JS runtime and is the identical RFC 9180
