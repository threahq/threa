...
test("keeps the sent-message menu open beside its reminder choices", async ({ page }) => {
  ...
  const timeInput = page.locator('input[type="datetime-local"]:visible')
  await expect(timeInput).toBeVisible()
  await expect(rootMenu).toBeVisible()
  await expect(timeInput).toBeFocused()
  await timeInput.fill(await timeInput.inputValue())
  const setTimeButton = page.getByRole("button", { name: "Set" })
  for (let i = 0; i < 8 && !(await setTimeButton.evaluate((button) => button === document.activeElement)); i++) {
    await page.keyboard.press("Tab")
  }
