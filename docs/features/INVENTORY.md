...
| [stream-descriptions](public/stream-descriptions.md) ✅                        | Rich-text description per stream, `description_set` timeline row, public-API + session-create write paths                                       | done                                                                         |
| [messages-and-timeline](public/messages-and-timeline.md) ✅                    | Message lifecycle: edit/delete, edited indicator, context menu and action drawer, system events, read-state controls, unread + day dividers      | done                                                                         |
| [message-composer](public/message-composer.md) ✅                             | Rich text, mentions, emoji, snippets, formatting toolbar + "+" overflow, fullscreen document mode                                               | done                                                                         |
| giphy                                                                          | GIF search and picker, GIF previews on messages (covered briefly in message-composer; lives in `components/giphy/`, not `features/giphy`)        | `components/giphy/`, `components/editor/giphy-*`                             |
...
| [slash-commands](public/slash-commands.md) ✅                                  | Command kinds (workspace/local/bot-runtime), stream-scoped availability, `/` palette, argument popover, pasted-command gate                      | done                                                                         |
...
| drafts                                                                         | Autosave, stashing, drafts page (the user-facing surface; cross-device roaming mechanics live in [architecture/drafts](architecture/drafts.md)) | `stores/draft-store.ts`, `pages/drafts.tsx`                                  |
| labels                                                                         | Private per-owner labels (color + emoji), assign to streams, name-based public API                                                              | `features/labels`, `pages/labels.tsx`                                        |
| memory-explorer                                                                | Browse and search memos, tags, knowledge types, memo embeds in messages                                                                         | `features/memos`, `pages/memory.tsx`                                         |
| [activity-feed](public/activity-feed.md) ✅                                    | All/unread/me views (URL-derived), mark read, unread counts derived from the held activity set                                                  | done                                                                         |
| [board](public/board.md) ✅ (building)                                         | Cross-stream wall of conversations by recent activity, read-only, flag-gated (`board-view`)                                                     | done                                                                         |
...
| [sync-log](architecture/sync-log.md) ✅                       | Durable per-workspace ordered log, delivery groups, group-filtered catch-up, heartbeat, retention                                    | done                                                                                            |
...
| [drafts](architecture/drafts.md) ✅                           | Drafts that roam across devices: split-on-drift CAS, push/pull over the offline queue, resolve-on-send, thread re-pointing, E2E seal | done                                                                                            |
...
| [claude-code-channel](architecture/claude-code-channel.md) ✅ | Drive a local Claude Code session from a Threa scratchpad over the bot-runtime API (one concrete bot-runtime kind)                    | done                                                                                            |
| [e2e-enclave](architecture/e2e-enclave.md) ✅ (building)      | SSK wraps, enclave instance keys, HPKE, the sealed AI loop                                                                           | done                                                                                            |
| [feature-flags](architecture/feature-flags.md) ✅             | Per-user enum-valued runtime flags: code registry, backoffice controls, CP→region fan-out, live broadcast                            | done                                                                                            |
