...
- **Flags needed before the bootstrap exists.** A flag that configures something built
  before any bootstrap is cached (the `SyncEngine`, say) can't read its value from the
  bootstrap directly. The pattern is to mirror the last delivered value into
  localStorage and recreate the dependent object when a newer delivered value differs —
  so a backoffice toggle still applies at runtime, at the cost of one recreation per
  change. No current flag needs this, so no such mirror exists today.
