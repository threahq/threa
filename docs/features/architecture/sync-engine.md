...
The data flows one way: the engine writes server state into **IndexedDB**, and the UI reads
from IDB via live queries. The engine's job is to make sure IDB is correct and current —
including across the messy parts (reconnects, backgrounded tabs, partial failures).
...
latest persisted sequence, `syncMode: "append"`), and applies them in one batch
...
- When it does prune a `replace` window, the ceiling is the **max event sequence among the
  returned events**, not `latestSequence` — because `latestSequence` can outrun the last
  returned event (new events committed between the server's event query and its sequence
  query), and pruning to it would delete exactly the gap events.
