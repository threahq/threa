...
- **No "local-only / not-yet-synced" badge in v1** (deferred, post-Stage-4). Surface a small hoverable `local` chip on a draft that has never reached the server — `!baseVersion` **and** `Date.now() − clientUpdatedAt > ~30s`, so an actively-edited draft never flashes it. It's a neutral _state_ label ("Saved on this device — not yet synced"), not an error: the queue retries silently, so an unsynced draft may simply be offline, server-erroring, or not-yet-drained, and we don't classify which. Notes for whoever picks it up: the signal already exists (`baseVersion` + `clientUpdatedAt` + the pending `upsert_draft` op); the 30s reveal needs a tiny `useStaleAfter(clientUpdatedAt, 30_000)` timer hook since Dexie reactivity is event-driven, not time-driven; the chip must overlay or reserve space (INV-21, no layout shift); natural surfaces are the stash picker rows and the `/drafts` explorer. Only meaningful once Stage 4 makes E2E drafts sync (as ciphertext) — before then encrypted drafts are deliberately local-only and must be excluded or they'd all read as "failed".
