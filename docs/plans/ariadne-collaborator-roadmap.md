...
- **"Delegations" in the stream-context surfaces** (Kris, 2026-07-10, follow-up to #1261) — surface a stream's delegations as their own section/filter chip in the "In this stream" panel, the way captured memories appear there (`stream-context-panel.tsx`, chip + `StreamContextRow` variant; row shows title + live status, taps through to the timeline card). Same authoritativeness caveat as the Reminders item below: memories can be timeline-derived because `memos:captured` is the whole story, but a delegation's **status lives in `delegation:status_changed` patches** — deriving from the loaded window would freeze out-of-window delegations on a stale status. So: an access-gated `GET /api/workspaces/:workspaceId/delegations?streamId=` over a `DelegatedTaskRepository.listByStream` read (mirror the cancel handler's `checkStreamAccess` gate) + invalidate on the `stream:delegation_created`/`stream:delegation_status_changed` socket events (both already delivered). Slots naturally with 5.3 (which adds list surfaces anyway) or right after.
