...
- Agent memory writes are never silent (INV-69 capture events); access flows through `checkStreamAccess`/`computeAgentAccessSpec`, never raw `stream_members` (INV-62).
...
| 5.4  | claude-code-remote delegation support                                          | ☑      | #1304, #1307   |
...
- Broadcast event `stream:brief_updated` (constants + payload + append in the same txn as the write): who, version, `reason`, and a short diff summary — brief changes are never silent (INV-69 spirit).
...
- **Dedicated-table decision confirmed; recovery behavior later superseded the original expiry transition.** Mechanics mirror bot-invocations (CAS transitions, TTL'd claim, sha256 token-at-rest via `hashCallbackToken`). The set-based lease sweep now reopens lapsed `claimed` or `running` rows and clears claim-holder fields (INV-56), appending each card patch in the same transaction. It does not automatically reopen historical rows already stored as `expired`; those use an explicit recovery path.
...
- **Superseded recovery ruling:** `expired` is recoverable, not terminal. Newly lapsed `claimed` or `running` rows reopen automatically; historical rows already stored as `expired` are not deployment-reopened, but can be claimed directly by id or explicitly requeued, marked done, or cancelled. Open-list deltas use `status_changed_at`, so reopened work appears in poll results. Inspect-before-claim and explicit release are now part of the public lifecycle.
...
**PR A shipped (server side of the push ruling):** `stream:delegation_created` now also emits a slim `delegation:available` nudge to the workspace runtime room (`bot:{workspaceId}`) on the `/bot` namespace (`BroadcastHandler.broadcastEvent`); claim accepts an `idempotencyKey` persisted before the request. A retry bearing the current unexpired claim's key re-keys it with a fresh token and lease via `reclaimByIdempotencyKey`. A lapsed-but-unswept claim cannot be re-keyed and waits for the sweep to reopen it. List accepts `since` for delta polls. Remaining for this step: the SDK `DelegationRunner` + `DelegationClient` (PR B).

**PR B shipped (client-side), then recovery semantics superseded its shutdown behavior:** `DelegationClient` remains the sibling client for user or bot keys and supports custom inspect-before-claim flows. `DelegationRunner` claims directly from its nudge and open-list paths, then uses heartbeat → injected `DelegationExecutor` → complete or fail, with single-flight execution and an awaitable `stop()` method. Controlled shutdown now aborts cooperatively and releases a live claim; executor errors still fail it. A known-lost claim aborts best effort without a terminal write. `BotRuntimeTransport` handles `delegation:available`, and `RemoteSession` exposes it as `onDelegationAvailable`. `claude-code-remote` remains the thin actuator; `pi-remote` adoption stays deferred per #1153.
...
**Current handover ruling:** claim transfer and handover remain deferred. Dogfood inspect-before-claim and release/reopen recovery before designing a second ownership protocol.

### 5.x Dogfood findings — delegation follow-ups (Kris, 2026-07-13, unscheduled)

Three findings from running the production user-key flow in a DM stream. Recorded with code-verified root causes; shapes below are candidates, not decisions — Kris's framing on the first: "that's a massive change in how things work, so we need to tread lightly."

**F1 — Result delivery lands as a wall of text, authored as the user.** `complete` posts the full `resultMarkdown` as an ordinary stream message; with a user key it's authored as the delegating user (5.3's two-branch identity, working as designed — and still uncomfortable: "it felt really awkward when the agent posted a massive slab of text… posted as messages, by me"). What he wants directionally: the agent comments on the delegation card itself — which means replying to non-message timeline events, a data-plane change to scope carefully. Interim candidates that need no data-plane change: (a) complete posts a compact status anchor message and the full result as a thread reply under it ("a super-duper small status update… and the actual message is sent on the thread to that"); (b) render-side collapse of long results. The card already links `result_message_id`, so any shape keeps the card→result pointer.

**F2 — Bots cannot be added to DM streams, and the failure is silent.** `StreamService.addBotToStreamOn` rejects DMs outright (`DM_MEMBERS_IMMUTABLE`, `streams/service.ts`) while the stream-settings members tab offers bot management on every stream type that has the tab (`members-tab.tsx` `canManageBots`) — a UI/server mismatch. Delegation consequence: a bot key's `GET /delegations` filters to `getAccessibleStreamIdsForBot` (channel grants), so a delegation created in a DM is **invisible** to the bot key — claim 404s, nothing happens, no signal anywhere ("you barely did anything and just didn't do anything"). Kris's ruling: DM _contacts_ stay immutable, but a bot should be addable as a participant to a DM — and to any root-level stream — so a personal bot can see and claim work there.

**F3 — No access-request handshake.** When a runner lacks stream access it currently learns nothing (F2's invisibility) and the human learns nothing either; fixing it takes a support conversation. Wanted: the claim/list path can surface "permission missing" with a request-access affordance — the bot files an access request, members with write permission on the stream get an approve/deny card, approval grants access and the runner proceeds. Depends on F2 (there must exist a grant to request). New substrate: request row + timeline card + approve flow.

Ordering note: F2 is the unblock (smallest, backend guard + UI truthfulness + grant semantics decision), F1 is the UX fix with an interim thread-shaped option, F3 is new substrate on top of F2.

**Status:** F2 shipped in #1316 (DM grants + participation-implies-visibility + Kris's mentionable ≠ invocable ruling: `findInvocableByIds` hard-blocks non-owner mention dispatch, amber "Mention only" autocomplete badge + chip). F1 shipped as interim shape (a): `complete` posts a compact anchor (`✓ Completed: **<title>**. Result in thread.`) and the full result as a thread reply under it, all in the completion transaction; `result_message_id` points at the anchor. Card-comment (reply-to-non-message-events) stays the directional goal. **F3 shipped** (`feat/bot-access-requests`): bot_access_requests substrate, public `request-access` (existence-hiding carve-out via the workspace nudge), member-gated first-party approve/deny, approve = grant + patch + re-nudge in one transaction, runner auto-request once per delegation, timeline card with member-gated actions; live-verified end to end (re-nudge→claim 14ms). **F4 shipped** (`feat/delegation-links`, stacked): canonical `/w/:ws/delegations/:id` URL + redirect route, delegation link kind in both parsers, access-tiered resolver (`target_delegation_id` persisted), first-party GET by id, title chip + status-pill preview card, Copy link on the delegation card. Still open from the F2 demo: author-relative mention chips, member-visible bots list, DM contact remove-X mismatch, active-scratchpad owner-only ruling, mobile bot profile view.

**F4 — Delegation links: copy, chip, card preview (Kris, 2026-07-13, follow-up).** "I can copy a link to a task or an agent delegation. It shows the title of the agent delegation in the chat, similar to how conversations work. And then, again, similar to conversations, the preview is basically the agent delegation card." Candidate shape, grounded in the existing in-app-link machinery (`apps/frontend/src/lib/in-app-links.ts` link kinds → `InAppLinkInline` chip via `useInAppLinkChip` local-cache→access-tiered resolution → `in-app-link-preview-card` rich preview): (1) a first-class delegation URL (new `kind: "delegation"` in `parseInAppLink`, frontend + backend `url-utils.ts` — a dedicated shape like `/w/:ws/delegations/:id`, not the ambiguous `?m=` message form); (2) a Copy link affordance on the delegation card next to Copy prompt / Mark done; (3) the chip resolves the delegation title + status through an access-gated lookup (`checkStreamAccess` on the delegation's stream, 404-as-invisible like the public API); (4) the below-message preview renders the existing delegation card component from the same data the timeline card uses. The F1 anchor message could then link the card instead of restating the title.
...
- **`memo_type` stays `'message'`, not a new value.** The tool requires ≥1 `sourceMessageIds` (Zod `.min(1)`), sets `source_message_id` to the first and `source_message_ids` to all, so the row satisfies the existing `memo_type_source` CHECK unchanged. The 6.3 session-sourced CHECK alternative is deferred to 6.3 as specced.
- **Source ids are scoped to the turn's own stream family (INV-8/INV-62 — no access widening).** Memos inherit retrieval access from their source stream (the researcher and explorer only ever surface a memo whose source/root stream is in the caller's accessible set — there is no workspace-wide memo read). `sourceMessageIds` is the one LLM-supplied field, so `saveMemo` resolves it via `MessageRepository.findByIdsInStreams(workspaceId, ids, sourceStreamIds)` where `sourceStreamIds` is `[addressed stream, its effective root]`. A cited id outside that family — another workspace, an inaccessible stream, or a _broader_ stream the user also happens to see — is dropped, never persisted as `source_message_id` (which would widen the memo's inherited access beyond the producing stream) nor folded into `participant_ids`. Only in-family ids anchor the memo; if none resolve the save is refused. Result: an agent memo is bound by exactly the same access scoping the passive pipeline would give the same conversation — never wider.
...
- **Capture event `conversationId` is now optional.** An agent memo is message-sourced with no owning conversation, so the `memos:captured` payload omits `conversationId` and the row links back through `memos[].sourceMessageIds`. The primary timeline (`event-item.tsx` → `memo-captured-event.tsx`) and `stream-context/derive.ts` render by event type / `memos[]` with no `conversationId` dependency, so INV-69 "visible in situ" holds on the main surface. The board's per-conversation card grouping (`board-event-rows.ts`) matches `memos:captured` rows by `conversationId`, so an agent memo (no conversation) does not attach to any board conversation card — by design, since it has no owning conversation to group under, not a regression of an existing behavior.
...
- **Privacy category is `workspace`, not `messaging`.** This gates tool AVAILABILITY, not memo visibility: `save_memo` is a write into the workspace knowledge layer (symmetric with `describe_memo`'s read), so it takes the `workspace` grant — a scratchpad that denies workspace tools opts out of the knowledge layer entirely (no memo reads, no memo writes). Memo _visibility_ is handled separately by source-stream inheritance (the anchoring above), not by this category. (The passive GAM pipeline is gated separately by the stream's `memory_mode`.) Ariadne gains `save_memo` in `enabledTools`; the researcher sub-agent never gets the callback. Enclave/E2E parity omitted, consistent with the other durable-write tools.
...
- **Reflective memos are message-anchored, not a new `memo_type: 'session'` — so no `memo_type_source` CHECK change and no retrieval-path edit.** The Shape called for a session-sourced CHECK alternative, but a `memo_type = 'session'` memo has no source message/conversation, so it resolves to no stream in the retrieval joins (`hybridSearch`/`semanticSearch`/`fullTextSearch`/`findNearDuplicate` all scope via `source_message_id → messages → streams` / `source_conversation_id → conversations → streams`) — it would be captured but never retrievable unless that INV-62-critical access path grew a session→stream branch in ~8 places. Instead the capture anchors each memo to the session's own in-stream message (the real trigger, else the last real reply, resolved in `buildSessionDigest`), so it stays `memo_type: 'message'`, flows through existing retrieval unchanged, and its inherited access is exactly the producing stream's (INV-8/INV-62 — never wider). `authored_by_kind: 'agent'` + `source_session_id` (the 6.2 columns) carry the provenance the goal asked for. Consequence: the one case with no real anchor — a fired follow-up (synthetic `followup_…` trigger) that did research but never replied — is skipped rather than captured under an unretrievable type; every user-triggered or replying session (the overwhelming majority, including research-without-reply on a real trigger) is covered.
- **The write reuses the 6.2 pipeline as `MemoService.captureSessionReflection` (INV-35).** Same three-phase shape as the batch (read stream memo context / classify + memorize + embed with no connection held, INV-41 / save under the per-stream `memo-batch:${streamId}` advisory lock, INV-20), reusing `findNearDuplicate` dedup and the `memos:captured` broadcast event (INV-69, no `conversationId` — links via `sourceMessageIds`). `MemoServiceLike`/`StubMemoService`/barrel updated. `MemoClassifier.classifyConversation`'s first param was narrowed to `ClassifiableConversation` (`Pick<Conversation, "id" | "topicSummary" | "participantIds">`) so the digest can drive it without fabricating a `Conversation`; the classifier runs with `topicSummary: null` (the digest's own section labels carry the framing).
- **The orchestration + digest live in `agents/`, not `companion/session.ts`.** The enqueue sits in `persona-agent-worker.ts` right beside 3.1's episode-summary enqueue (the actual post-completion spot), fired for every completed session with a `sessionId` (the replyless research turn the episode gate skips must still be covered; the service self-gates cheaply). `ReflectiveCaptureService` (mirrors `EpisodeSummaryService`) owns the session gate + digest + once-only claim; `MemoService` owns classify→memorize→write. The session digest that both the summarizer and the reflective capture read is now one shared `buildSessionDigest` (`agents/session-digest.ts`) — `EpisodeSummaryService` was refactored onto it (INV-35), and it also reports `hasResearch` (the reflective gate: no `turn_digest` findings ⇒ no capture, so ordinary chat isn't double-classified against the passive pipeline) and the `anchorMessageId`.
- **Idempotency is a `reflective_captured_at` CAS column on `agent_sessions` (new migration), not the `memo_type_source` CHECK migration.** The memorizer is non-deterministic (temperature 0.3), so a re-delivered job's differently-worded memo can fall outside the `MEMO_DEDUP_DISTANCE` gate — the embedding dedup alone can't guarantee once-only. The CAS is claimed _before_ the classifier runs so a concurrent/redelivered delivery can't stack a second capture; the claim is taken even when the session yields no memo, so a not-worthy or non-research session isn't re-classified on every redelivery. Because the claim precedes the fallible AI + save work (unlike `setEpisodeSummary`, whose CAS is its terminal write and so leaves the marker NULL on a mid-flight failure), `ReflectiveCaptureService.capture` wraps that work in a `try/catch` and calls `clearReflectiveCaptured` to release the claim on any error — the memo writes are transactional, so a failure committed nothing and a retry recovers, while the no-duplicate guarantee holds (only the delivery that won the atomic claim reaches the release). The residual loss window is a process crash between claim and release; the correctness trade is "never duplicate a durable memo" over "never miss one."
- **Config cap `MEMO_REFLECTIVE_MAX_MEMOS = 2`** in `memos/config.ts` (tighter than `MEMO_MAX_PER_CONVERSATION`; agent memos are damped by `MEMO_AUTHORED_BY_KIND_BOOST` anyway). New job `AGENT_REFLECTIVE_CAPTURE` + `AgentReflectiveCaptureJobData`, worker `createReflectiveCaptureWorker`, `server.ts` wiring (light tier, alongside the episode worker; `reflectiveCaptureService` binds `memoService`).
...
- **Auto-user-scoping is private-scratchpads-only, not DMs.** The Shape said "DMs/private scratchpads default to `user` scope for the owning user," but a DM has two participants and no single owner — assigning `user` scope to one would wrongly hide shared knowledge from the other, while a DM's existing stream-access filtering already keeps its memos out of any non-participant's context (the footgun the step guards). A private scratchpad has exactly one owner (`streams.created_by`), so it is the unambiguous solo-first "about you" surface: the passive extractor (`resolveExtractedMemoScope` in `memos/service.ts`) files its memos as `user`/owner, and DMs/channels/public scratchpads stay `workspace`-scoped. Reflective capture (6.3) inherits the same per-stream tier.
- **The scope predicate lives on the `MemoSearchFilters` gate, applied in every memo search path — not just `hybridSearch`/`semanticSearch`.** `fullTextSearch` (both branches) and `exactSearch` are live retrieval fallbacks, so a `user`-memo would leak through them if only the two named paths were gated. All five carry the same two-part predicate (`viewerUserId` visibility gate + optional positive `scope` filter) pushed into the inner access-scoped scan, never as a post-filter (§3.1). `findNearDuplicate` is scoped to the candidate's own tier so a private memo can't dedup against a shared one (or vice versa).
- **The invoking user is threaded directly, `access-spec.ts` untouched.** The Files list named `researcher/access-spec.ts` ("carry invoking user"), but `WorkspaceAgentInput.invokingUserId` is already in the researcher's call chain — the scope viewer is user identity, orthogonal to the spec's stream-access job — so `searchMemos` passes `viewerUserId` into the filters directly rather than reshaping `AgentAccessSpec`.
- **`save_memo`'s scope arg defaults to the save stream's natural tier.** The tool exposes an optional `user`/`workspace` scope; omitted, the memo inherits the stream tier (a private scratchpad save is private), matching passive extraction. An explicit `user` scope is owned by the invoking human (`agentContext.invokingUserId`); with no invoking user it falls back to `workspace` rather than mint an ownerless (CHECK-violating) user memo. `stream` scope is not tool-exposed.
- **Delete is a real hard-delete gated to user-scoped memos the caller owns** (`DELETE /memos/:id`, `MemoRepository.delete`) — the "forget what you know about me" action. Shared (`workspace`/`stream`) knowledge stays archive-only (403); the explorer's `resolveAccessibleMemo` owner-gate blocks viewing/editing/deleting another user's private memo even with source-stream access. Public-API memo reads pass the principal user for user-key requests and no user for bot keys (bots never see the private tier). A `memo_scope_owner` DB CHECK enforces `user ⟺ scope_user_id`.
...
- **v1 edits Ariadne only — no create/archive of new personas.** The one visible built-in is editable; `GET /personas` lists it, `GET /personas/:id/config` + `PUT /personas/:id/override` read/write its sparse patch over the code defaults (`applyBuiltInAgentPatch`, `agent_config_overrides`, race-safe `ON CONFLICT` upsert with optimistic-concurrency 409). `EMPTY_AGENT_ID` and any non-visible/unknown id are 404 everywhere. The full persona lifecycle (workspace-created personas, archive) stays open for a later slice.
- **No `status` field in the API patch.** The shared editable-field schema is single-sourced in `packages/types/persona-config.ts` (INV-31), but the API write variant withholds `status` (a `.strict()` extra-key rejection), so an admin cannot disable or archive Ariadne through the editor in v1. The backend's internal override-resolution schema keeps `status` for a stored override that carries it — resolution must accept whatever is already persisted.
- **Supersedes #1282's interim model-override tab.** #1282 shipped a stopgap "model only" control in an Agents settings tab; this replaces its editor surfaces (that tab, its `PATCH …/config` endpoint, and the `mergePatch`/`removePatchKeys` repo methods) with the full editor. #1282's companion-eval fixes, `claude-sonnet-5` registration, and the Ariadne sonnet-5 default were kept.
- **Registry-derived model allowlist (INV-16 now enforced for overrides).** The assignable-model set is derived server-side from the `ModelRegistry` (the text-in/text-out chat models — embeddings and realtime STT excluded), delivered on the config response as `availableModels` and re-validated on `PUT …/override` + `PUT …/draft` (400 `UNSUPPORTED_PERSONA_MODEL`); the shared Zod schema keeps shape validation only. A persona's own built-in default models stay assignable even if the registry lacks them. The editor no longer ships a hardcoded model list.
- **Test-drive substrate the roadmap didn't spec.** "Test before commit" is mandatory UX: the editor holds a server-side _draft_ (`persona_config_drafts` tracking table, INV-57 — `UNIQUE(workspace, agent, creator)`, never written to `agent_config_overrides` until explicit Save) and a bound ephemeral test scratchpad (real stream, companion mode on, **memory OFF** so test chats never feed GAM). Debounced edits sync into the draft; a live companion turn in the test stream runs against the draft config so the admin talks to the candidate Ariadne first.
  - **A new invocation variant is a new `TurnPurpose` member (roadmap 1.5), not an optional flag.** `personaDraftId` rides on `PersonaAgentJobData`; `CompanionHandler` (plaintext path only) tags a turn whose stream is some draft's `test_stream_id`; `resolveTurnPurpose` maps it to `{ kind: "draft_test", draftId }`, and the precheck resolves the persona as `applyBuiltInAgentPatch(base, draft.patch)` for that turn. A draft that vanished mid-chat (committed/discarded) falls back to normal resolution — never fails the turn.
  - **Fidelity rule: no "this is a test" prompt section.** The draft `systemPrompt` runs verbatim; the whole point is judging real behavior. `deriveTurnFlags` is unchanged.
  - **Safety rule: durable-write tools are stripped in `draft_test` turns** via the named `DRAFT_TEST_EXCLUDED_TOOLS` (`features/agents/config.ts`) — follow-up mutators, `update_stream_brief`, `delegate_task`, `save_memo`. Read tools, `send_message`, `react_to_message` stay. (memory-off already stops GAM capture; this stops tool-side durable writes.)
  - **Save and Discard both archive the test scratchpad.** Save commits the override, drops the caller's draft in the same txn, and archives the bound stream (best-effort, after commit — a 409 never kills the chat); Discard archives the stream then deletes the draft row. An archived/vanished bound stream reads as "no active test chat" across reload.
- **Editor is a route-level page** (`/w/:workspaceId/settings/personas/:personaId`, admin-gated, persona id in the URL per INV-59), reached from a new Personas tab in the workspace-settings dialog. Per-field overridden-vs-default badges + reset; commit uses optimistic concurrency with an inline conflict banner (keep-draft / load-theirs, no toast, INV-63). This is the **editor slice of 7.2** landing early with 7.1; the per-stream picker (`companion-tab.tsx`) is still open.
- **Future seam kept open: role-play a draft persona inside real streams.** Draft resolution is stream-agnostic (the precheck substitution point is the single seam), so a later slice can run the candidate persona in an existing stream rather than only a dedicated test scratchpad — not built now, but not painted out.
- **Staging-review UX revisions (#1285 follow-up):**
  - **Test scratchpad excluded from the sidebar via a first-class `streams.purpose` marker.** The bound test stream was showing up as a normal scratchpad in the creator's sidebar. There is no stream-level hide mechanism (the #1196 hide/mute is board/conversation-scoped), so a new nullable `TEXT purpose` column on `streams` (INV-3 validated in code via `StreamPurposes`, no enum; migration `20260711120000_stream_purpose.sql`) marks it `'persona_test'` at creation. The marker rides the `Stream` wire type, so **both** population paths agree: the workspace bootstrap query (`StreamRepository.listWithPreviews`) filters `s.purpose IS NULL` in every branch, and the live `stream:created` socket handler (`workspace-sync.ts`) early-returns for a purposed stream so it never enters the sidebar cache or IDB. A cross-table NOT-EXISTS on `persona_config_drafts` was rejected: it can't reach the client's live socket add (the client can't know the id is a draft's test stream). The stream stays fully functional mounted directly — `findById`/per-stream bootstrap are unfiltered — so the desktop test pane and the mobile route keep working. Archived-stream views inherit the same exclusion (acceptable v1).
  - **System prompt (and only that field) edits in a `RichEditor`, not a raw `Textarea`.** Reuses the markdown round-trip pattern from `settings/ai-settings.tsx` (`RichEditor` + `EditorActionBar` + `serializeToMarkdown`/`parseMarkdown`, mentions/channels/commands/emoji disabled). The patch still stores a markdown **string** — no schema/API change; `values.systemPrompt` stays the source of truth for the sparse-patch/dirty/counter machinery, and a `lastSerializedPromptRef` guards the re-seed effect so an external baseline swap (reset / discard / adopt-theirs / broadcast) re-seeds the doc while the user's own keystrokes do not (no cursor churn, no loop). The char counter runs on the serialized markdown against `PERSONA_SYSTEM_PROMPT_MAX_CHARS` and blocks Save when over (the backend 400s anyway). Description stayed a plain `Textarea` — swapping it was not trivial (nullable short label) and the brief gated that on "if trivial".
  - **Mobile "Test draft" opens a drawer, not a route.** It navigated to the test stream as a normal route, yanking the admin out of editing. Now a `vaul` bottom `Drawer` (~92dvh, the app's `ui/drawer` which already runs `repositionInputs={false}` so `StreamContent`'s own mobile keyboard choreography keeps the composer above the keyboard) mounts the SAME test-chat content as a sibling of the form — the form never unmounts, so draft state survives open/close; swipe-down/close is not "End" (the bound session persists and a reopen resumes it). The create/archive lifecycle is a shared `usePersonaTestSession` hook + `PersonaTestChatEmptyState` used by both the desktop pane and the mobile drawer. Desktop split view is unchanged.
...
- ☑ **"Delegations" in the stream-context surfaces** (Kris, 2026-07-10, follow-up to #1261) — **shipped in #1269** (supersedes the backlog-entry doc PR #1268). A stream's delegations render as their own chip + `StreamContextRow` variant in the "In this stream" panel: title, live status pill, `claimedByLabel`/`statusNote`, tap-through deep-linking the timeline card by its `delegation:created` event id. Authoritative as designed (statuses live in `delegation:status_changed` patches): access-gated `GET /api/workspaces/:workspaceId/delegations?streamId=` over `DelegatedTaskRepository.listByStream` (LEFT JOIN returns `createdEventId`), invalidated by stream-sync on both delegation socket events. Also landed: `?m=` now deep-links `event_…` ids (the `events/around` `eventId` param was already there server-side), which the Reminders item below can reuse. Row is a pointer, not a mini-card — Copy/Cancel/result stay on the card.
