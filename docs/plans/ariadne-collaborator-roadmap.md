...
**PR A shipped (server side of the push ruling):** `stream:delegation_created` now also emits a slim `delegation:available` nudge to the workspace runtime room (`bot:{workspaceId}`) on the `/bot` namespace (`BroadcastHandler.broadcastEvent`); claim accepts an `idempotencyKey` (persisted pre-request; a retry bearing the live claim's key re-keys it — fresh token + lease via `reclaimByIdempotencyKey`, no status guard on expiry so the lapsed-but-unswept window is recoverable); list accepts `since` for delta polls. Remaining for this step: the SDK `DelegationRunner` + `DelegationClient` (PR B).

**PR B shipped (client side):** `DelegationClient` (the recorded sibling-client ruling: user or bot key, `X-Threa-Callback-Token` transport, idempotency-key claims) + `DelegationRunner` (nudge-or-poll → claim → heartbeat → injected `DelegationExecutor` → complete/fail, single-flight, awaitable `stop()`) in `extensions/remote-session`; `BotRuntimeTransport` handles `delegation:available` and `RemoteSession` exposes it as `onDelegationAvailable`. `claude-code-remote` contributes only the deliver-to-Claude actuator: opt-in via `THREA_DELEGATIONS=1` (off by default — delegations are workspace-wide, so only an explicitly opted-in connector should race for them), the brief lands as a `<delegation>` channel event, Claude's `send`s become the card's progress note, its `reply` completes the card with the result; idle-timeout reaping and graceful shutdown both fail the claim loudly instead of stranding it to the lease sweep. `pi-remote` adoption stays deferred per #1153.
