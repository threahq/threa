...
**Status:** F2 shipped in #1316 (DM grants + participation-implies-visibility + Kris's mentionable ≠ invocable ruling: `findInvocableByIds` hard-blocks non-owner mention dispatch, amber "Mention only" autocomplete badge + chip). F1 shipped as interim shape (a): `complete` posts a compact anchor (`✓ Completed: **<title>**. Result in thread.`) and the full result as a thread reply under it, all in the completion transaction; `result_message_id` points at the anchor. Card-comment (reply-to-non-message-events) stays the directional goal. F3 open. Also open from the F2 demo: author-relative mention chips, member-visible bots list, DM contact remove-X mismatch, active-scratchpad owner-only ruling, mobile bot profile view.
