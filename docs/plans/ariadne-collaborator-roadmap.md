...
- **Source ids are access-scoped (INV-8/INV-62).** `sourceMessageIds` is the one LLM-supplied field, so `saveMemo` resolves them via `MessageRepository.findByIdsInStreams(workspaceId, ids, accessibleStreamIds)` — a cited id from another workspace or a stream the invoking user can't see is dropped, never persisted as `source_message_id` nor folded into `participant_ids` (matching `react_to_message`'s gate). Only resolved ids anchor the memo and seed `participant_ids`; if none resolve the save is refused. `accessibleStreamIds` threads from the turn's context through the `saveMemo` bind.
