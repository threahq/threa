...
- **Enclave turns don't carry the brief.** `buildEnclaveSystemPrompt` is untouched — consistent with E2E parity being deferred across this roadmap (follow-ups, delegations). The brief row itself is plaintext server-side either way, so this is a prompt-assembly gap only, revisit with the sealed wire.
