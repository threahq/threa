...
One PostHog project per region, and a service reports to the project for the region it runs in, not the region its users are in. The `eu-north-1` backend and the control-plane both run in `europe-west4`, so both report to the EU project (`POSTHOG_HOST=https://eu.i.posthog.com`); a US-hosted backend would report to a US project (`https://us.i.posthog.com`). The control-plane is global in role but its data lives in the EU Postgres, so sending its telemetry to a US project would move EU personal data out of the region. Set both variables or neither: one without the other refuses to boot; neither in production boots with a single warning and error reporting disabled (INV-11).
...
| `POSTHOG_PROJECT_TOKEN`, `POSTHOG_HOST` | EU PostHog project token + `https://eu.i.posthog.com` — the control-plane runs in `europe-west4`; see the backend PostHog section                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
