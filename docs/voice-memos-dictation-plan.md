...
> **Revision note (streaming + steering):** The transport is a **realtime WebSocket transcription session** (true word-by-word streaming) — there is no batch/async path. Default provider is **ElevenLabs Scribe v2 Realtime** (~150 ms latency, STT-specialized, zero-retention mode, $0.39/hr). The comparison/steering strategy is **Deepgram Nova-3** (strongest keyterm steering, EU residency region) — both swappable behind one interface. OpenAI was evaluated and ruled out (its prompt steering doesn't work in realtime sessions). Section 2 covers the data flow; §7.1 covers data residency. See [§2](#2-end-to-end-data-flow).
...
**Default model is `elevenlabs:scribe-v2-realtime`.** Real word-by-word transcription needs a realtime WebSocket STT session, so we only integrate providers that offer one. ElevenLabs Scribe v2 Realtime is purpose-built for streaming STT (lowest latency and cheapest of the realtime options at $0.39/hr, strong accent handling, context-aware keyterm steering, zero-retention mode that matches our no-persistence stance). **Deepgram Nova-3** is the comparison/steering strategy: the strongest keyterm steering available (100 words / 500 tokens, multilingual) and a usable EU residency region — the escape hatch when steering or data-residency needs outgrow ElevenLabs' starting tier. Both sit behind one `TranscriptionStrategy` interface, so swapping to compare is a settings toggle. There is **no batch/one-shot path** (no OpenRouter/Whisper integration) — it was cut to keep v1 as simple as possible.
...
4. **Strategy coverage in v1** — ship just the default realtime provider, or wire both providers from day one so you can A/B them immediately.
...
| **transcribing** | audio in flight, text arriving                             | waveform continues; subtle "·" indicator next to caret as text streams in                       |
| **finalizing**   | user stopped recording, last segment still being finalized | waveform freezes; pulse indicator until the final transcript lands                              |
...
- **Transcription model** — radio buttons: ElevenLabs Scribe v2 Realtime (default), Deepgram Nova-3 (heavy keyterm steering, EU region). Both realtime; there's no batch/offline option. (Models added to `models.yaml` and to a new "Speech-to-text" section in `docs/model-reference.md`.) A workspace residency policy (§7.1) can further constrain or hide options here — an EU-required workspace won't be offered a US-only model.
...
This is the meat of the question you flagged: **how audio gets from the mic to the model and how text comes back into the composer.** The only path is a realtime WebSocket transcription session (default provider ElevenLabs Scribe v2 Realtime) for genuine word-by-word streaming.

### 2.1 Why realtime only

True streaming — audio in, partial transcripts out continuously — needs a provider with a realtime STT WebSocket. One-shot/batch transcription (e.g. Whisper turbo via OpenRouter) can't drive live dictation and we explicitly chose not to build a batch path, so the two realistic providers are both realtime:

1. **ElevenLabs Scribe v2 Realtime** (recommended default) — WebSocket STT, ~150 ms last-chunk-to-text latency, 90 languages, strong accents, **zero-retention mode** ([Scribe v2 Realtime](https://elevenlabs.io/realtime-speech-to-text)). STT-specialized; currently tops accuracy benchmarks.
...
**Decision:** a realtime WebSocket session, defaulting to ElevenLabs. The cost is new provider integrations — `ELEVENLABS_API_KEY` and `DEEPGRAM_API_KEY` (the backend currently only configures `OPENROUTER_API_KEY` in `env.ts:152`) — plus a WebSocket relay on the backend. You've explicitly accepted that complexity in exchange for streaming.
...
Use the Web Audio API with an `AudioWorkletNode` to read raw `Float32Array` samples, downsample to 16 kHz mono, and convert to PCM16: emit ~100 ms PCM16 frames continuously to the relay (binary socket.io events); the per-provider strategy wraps each frame in that provider's wire format upstream (ElevenLabs and Deepgram each take raw/binary PCM frames over their WS, which is why the framing detail lives inside the strategy, not the client). Server-side VAD handles segmentation — no manual silence detection needed.
...
Both providers go through one interface so the provider difference is invisible to the rest of the feature:

```ts
interface TranscriptionStrategy {
  // Opens an upstream WS, returns a live session.
  open(opts: { model: string; language?: string; vocabulary?: string[] }): Promise<TranscriptionSession>
}

interface TranscriptionSession {
  pushAudio(frame: Int16Array): void // append a PCM16 frame to the upstream WS
  flush(): Promise<void> // ask upstream to commit any buffered audio
...
The interface is genuinely uniform now that both implementations are realtime: each opens an upstream WS, takes `pushAudio` frames, and emits `onDelta`. The only differences (frame wire format, event names, auth, keyterm payload) live inside the two strategy classes; the gateway never branches on provider.
...
**The two providers share the relay shape**: client frames → backend relay → upstream WS → deltas back. Adding Deepgram cost nothing architecturally — it's the payoff of the strategy interface, and it directly serves your "compare which is most useful" goal (ElevenLabs vs Deepgram realtime), with Deepgram covering the heavy-steering / EU-residency cases ElevenLabs can't on our starting tier.

The wrapper question (INV-19/INV-28): neither provider's SDK is wired into the project AI wrapper, so each strategy does its own WS. To stay inside the project's AI-governance, expose them through a `createAI`-adjacent factory (`createTranscription({ elevenlabs, deepgram })`) that owns telemetry metadata and `CostRecorder.recordUsage()` — same pattern as `createAI`, just a different modality. Cost is recorded on `close()` from the upstream usage event (both report billed audio duration).
...
A new `audio` modality and a `streaming: realtime` capability are introduced; `ModelRegistry` gets `supportsAudioInput()`. Update `docs/model-reference.md` with a "Speech-to-text" section: ElevenLabs Scribe v2 Realtime (~$0.39/hr), Deepgram Nova-3 (~$0.46/hr streaming), and "when to use" guidance (ElevenLabs realtime for dictation default; Deepgram realtime when heavy keyterm steering or EU residency is needed).

### 2.5 Backend → client (live transcript)

Deltas arrive on the relay WS and are forwarded to the browser as `voice:transcript:delta` events `{ voiceSessionId, text, isFinal }`, emitted directly to the originating socket (the relay already holds it) with **no outbox hop**. The frontend listens only while a session is locally active and filters by `voiceSessionId`.

> **Explicit deviation from INV-4 — needs sign-off.** INV-4 ("real-time delivery goes through the outbox, not ad hoc publish calls") is a hard architecture boundary, so bypassing it is a deliberate decision, not an oversight. The justification: transcription deltas are transient, per-connection, sub-second, and produced by the same process that holds the client socket — they are not durable workspace state and there is no event-sourced projection to keep consistent (INV-7 doesn't apply). Routing 10 deltas/sec/user through the outbox would add latency and DB write load for data that is meaningless once the session ends. If you'd rather not carve an INV-4 exception, the fallback is to keep realtime deltas on the relay socket but document the exception in CLAUDE.md as a named carve-out. Flagging this for your call before implementation.
...
### 2.6 Streaming text into the composer (UX layer)

When a delta arrives, we don't dump all the text at once — we want the "typing" feel.
...
- When the session ends (`isFinal: true` on the last delta and buffer empty), the buffer is cleared and the composer returns to idle.

Deltas already arrive word-by-word every ~200–400 ms, so the buffer mostly just smooths jitter into a believable typing animation.
...
├── config.ts                   # default models, frame sizes, language defaults
├── handlers.ts                 # session create/finish/abort
...
│   ├── strategy.ts             # interface + factory createTranscription({ elevenlabs, deepgram })
...
- `POST /api/workspaces/:wid/voice/sessions` — create a session. Body: `{ model?: string, language?: string }`. Response: `{ voiceSessionId, expiresAt }`. Defaults from user preferences, else `config.ts`.
...
| provider       | TEXT NOT NULL              | `elevenlabs` / `deepgram`                                      |
...
Index on `(workspace_id, user_id, status)` to find a user's active session quickly.
...
No job queue and no outbox: the realtime gateway relays frames synchronously over the live WS and emits transcripts straight back to the originating socket.

- **Transcript stream** (relay → originating socket, no outbox): `voice:transcript:delta` `{ voiceSessionId, text, isFinal }`, `voice:transcription:error` `{ voiceSessionId, code, message }`.
- `voice:session:expired` — emitted by a sweeper when an active session passes `expires_at`.

### 3.4 Cost tracking

Both strategies record on session `close()` via `CostRecorder.recordUsage()` with telemetry metadata `{ functionId: "voice_transcription", voiceSessionId, model, totalAudioMs }` (INV-19).

ElevenLabs and Deepgram both report billed audio duration on the session; convert to dollar cost via the provider's per-hour rate and record it. **The per-hour rates must not be hardcoded in cost code** (INV-33: centralize constants at the source of truth) — add an `audioPricePerHour` (or per-second) field to the relevant `models.yaml` entries and read it through `ModelRegistry`, so pricing lives in one place alongside the existing model metadata and can't drift from `docs/model-reference.md`.

Both bill on audio duration rather than text tokens. The `CostRecorder` may need a minor extension to record `{ unit: "audio_seconds", quantity }` alongside token usage.
...
│       ├── voice-transport.ts         # dedicated voice socket: emit frames, receive deltas
...
├── lib/api/voice.ts                   # createSession / finishSession / abort
└── pages/settings/voice-settings.tsx  # new settings panel
```

### 4.2 Recorder API

The recorder produces PCM16 frames and hands each one to the transport, which emits it on the dedicated voice socket.

```ts
const recorder = createVoiceRecorder({
  frameMs: 100, // send each ~100 ms frame as it's captured
...
const transport = createRealtimeTransport(voiceSocket, session.voiceSessionId) // emits voice:audio frames
```

### 4.3 Receiving deltas

Hook `useVoiceTranscriptionStream(voiceSessionId)` listens for `voice:transcript:delta` on the active voice socket; it handles both append and interim-replacement (track current interim span, overwrite until `isFinal`), filters by `voiceSessionId`, and pushes into the local type buffer. A dropped socket ends the session (see §2.5) — there's no partial replay (so no INV-53 bootstrap is needed; there is no decoupled completion stream to recover).
...
Tidy-up runs **once at end-of-session** — when the user stops dictating, the finalized transcript for that session is sent through a cleanup model and the composer's dictated range is replaced wholesale (TipTap range replace, animated). No per-segment rewriting, no streaming tidy. This keeps it off the realtime hot path entirely: the user gets the raw transcript live, and the polish lands a beat after they stop.

It rides the voice socket already open during the warm-linger window (§2.2), so no new transport or outbox event is needed — the backend sends a single `voice:transcript:tidied` frame carrying `{ voiceSessionId, originalText, tidiedText, span }` down the same socket. If the socket has already closed, the result is dropped (the user moved on).

### 5.2 The tidy-up call

When tidy-up is enabled, on session stop the backend calls `gpt-5.4-nano` (or `gpt-5.4-mini` per user preference) with a prompt that includes:

- The full finalized transcript for the session.
...
Lives in `voice-transcription/tidy/` to colocate (INV-51). Eval harness in `voice-transcription/tidy/evals/` uses the production entry point (INV-45). Config in `voice-transcription/tidy/config.ts` (INV-44). End-of-session means a single synchronous-ish call gated behind the user-stop event — no background worker or job queue is introduced for v1's tidy-up.
...
- **Audio bytes: never persisted.** Frames pass through the relay and are discarded. No S3 write, no temp file. Logs only durations + transcribed text, not audio. ElevenLabs zero-retention mode (and Deepgram's no-storage / no-training default) means the upstream provider doesn't retain audio either.
...
- **Audio in transit.** TLS to backend, TLS to the upstream provider (ElevenLabs/Deepgram). The relay terminates the client connection and opens its own upstream connection; the API key never reaches the browser. Prefer providers' zero-retention / no-training modes where available.
...
- **Rate limiting & concurrency.** Reuse the existing rate-limit middleware; max 1–2 concurrent sessions per user (the gateway holds one upstream socket each).
...
This gives you a concrete answer for residency-sensitive customers today (Deepgram-EU or disable), keeps the door open to ElevenLabs-EU once Enterprise is in play, and the abstraction means adding a third in-region option later (e.g. Azure Speech EU) is just another resolver branch.

---

## 8. Testing

- **Unit:** `voice-recorder.ts` (PCM framing/downsampling), `insert-stream.ts` (caret-respecting drain + interim-replacement), `service.ts` (session lifecycle + strategy selection), `residency.ts` (policy → provider/region resolution, including the fail-closed "disable" branch when no in-region strategy exists), each strategy in `transcription/` (frame relay, error mapping). Mock the upstream WS.
...
- **Deferred to later PRs:** interim-replacement/flicker handling, warm-linger socket, idle-timeout + 10-min expiry sweeper, error/resume UX, cost recording, residency, settings, Deepgram, waveform.
...
### PR 6 — Tidy-up pass (deferred follow-up)

Separate from v1 entirely. Small model cleans disfluencies/corrections at end-of-session (§5), replacing the dictated range wholesale once the user stops.

---

**Sequencing notes.** PRs 1→2 are the recommended order and each builds on the last, but **3, 4, 5 are independent of one another** once PR 2 lands — they can ship in any order or in parallel. v1 = PRs 1–5. If you want the absolute minimum first cut to play with, PRs 1–2 alone are a complete, shippable ElevenLabs dictation feature; 3–5 are hardening and breadth.
...
| Mobile Safari quirks with AudioWorklet on older iOS.                         | Capability-detect at session start; if unsupported, disable the mic with an explanatory tooltip rather than failing silently.                |
...
| Socket drop mid-session loses in-flight audio.                               | Sessions end on drop (no clean resume); preserve landed text and surface "tap to resume" for a fresh session.                                |
...
3. ✅ **Strategy coverage in v1 — start with ElevenLabs.** Ship ElevenLabs realtime first (PRs 1–2); Deepgram realtime lands in PR 5 (§9) so you can A/B and so the EU-residency path exists. There is no batch/OpenRouter strategy — realtime only.
...
5. ✅ **Tidy-up timing model — end-of-session, deferred until we build it.** The tidy-up pass is deferred (§5) and doesn't block v1. When we build it, it runs once on session stop and replaces the dictated range wholesale — no per-segment rewriting, keeping it off the realtime hot path.
...
- `apps/backend/src/lib/ai/ai.ts:30,392-949` — `createAI` wrapper; pattern to mirror for `createTranscription`
