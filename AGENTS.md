...
The npm scope is `@threahq` — `@threa` is not an account we own. Only the packages `publish-npm.yml` lists are publishable; every other `package.json` carries `"private": true`. `bun run check:package-scopes` enforces both and runs in `lint`.
...
- Timeline landing is atomic (INV-70). A stream open's initial viewport position is decided exactly once, by `resolveStreamLanding` (`apps/frontend/src/components/timeline/stream-content.tsx`) — priority deep-link › user-gesture › anchor restore › unread marker › tail — and executed by the single landing effect: positional landings take the cold-load settle over (hold the mask, position behind it, reveal once the target holds), so the first painted frame is at the landing, never a tail flash followed by a jump. Never add a second on-open scroll effect — stacked landers fighting each other is the recurring regression this exists to prevent. A new landing behavior adds a `StreamLanding` variant + resolver branch, a decision-table case, and a browser spec proving first-paint-at-landing (`timeline-anchor-restore`, `unread-marker-open`, `stream-switch-stability` are the references). Anything else that adjusts position on open (composer initial correction) must yield to an active `scrollToMessage` refine loop or a landed detached position.
...
- **Frontend/UX:** INV-14, 15, 18, 21, 40, 42, 53, 59, 60, 61, 63, 69, 70
