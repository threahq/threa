...
import { botApiPost, createBot, createBotKey, createWorkspace, loginAs, TestClient } from "../client"
import {
  base64ToBytes,
  buildMessageAad,
  buildWrapAad,
...
describe("sealed harness session (full E2EE round trip)", () => {
  test("harness-created encrypted scratchpad serves a sealed turn end to end", async () => {
    ...
    // The owner seals the trigger into the scratchpad.
    ...
    const sealedTrigger = await sealMessage({
      key: ssk,
      keyGeneration: 0,
      payload: serializeSealedPayload(triggerText),
    ...
    const interim = await sealReply(opened.sealing, interimText)
    ...
    const reply = await sealReply(opened.sealing, replyText)
    ...
    const openWireMessage = async (messageId: string): Promise<string> => {
      ...
      return parseSealedPayload(raw).contentMarkdown
    }
    expect(await openWireMessage(interim.messageId)).toBe(interimText)
    expect(await openWireMessage(reply.messageId)).toBe(replyText)
  })
})
