...
const PullsSchema = z
  .object({
    mode: z
      .enum(["list", "get", "files"])
      .describe(
        "list: pull requests in a repo (filterable). get: one PR with body, merge state, and review summary (requires number). files: files changed by a PR with optional diff patches (requires number)."
      ),
    owner: z.string().min(1).describe("Repository owner"),
    repo: z.string().min(1).describe("Repository name"),
    // get / files target
    number: z.number().int().min(1).optional().describe("Pull request number. Required for mode=get and mode=files."),
    // list filters
    state: z.enum(["open", "closed", "all"]).optional().default("open").describe("list: filter by PR state"),
    base: z.string().optional().describe("list: filter by base branch (e.g. main)"),
    head: z.string().optional().describe("list: filter by head user:branch"),
    sort: z
      .enum(["created", "updated", "popularity", "long-running"])
      .optional()
      .default("created")
      .describe("list: sort field"),
    direction: z.enum(["asc", "desc"]).optional().default("desc").describe("list: sort direction"),
    // files options
    includePatches: z
      .boolean()
      .optional()
  ...
  // optional at the schema level and required for the two single-PR modes here.
  .superRefine((value, ctx) => {
    if ((value.mode === "get" || value.mode === "files") && value.number === undefined) {
      ctx.addIssue({ code: "custom", path: ["number"], message: `number is required when mode=${value.mode}` })
    }
  })
...
async function getPullRequest(deps: GitHubToolDeps, input: PullsInput): Promise<AgentToolResult> {
  const result = await withGithubClient(deps, async (client) => {
    ...
    return {
      ...
      // github_commits (mode=list) for enumeration.
...
export function createGithubPullsTool(deps: GitHubToolDeps) {
  return defineAgentTool({
    name: "github_pulls",
    categories: TOOL_CATEGORIES_BY_NAME[AgentToolNames.GITHUB_PULLS],
    description: `Read pull requests in a GitHub repository. mode=list returns PRs (filter by state/base/head, sort, paginate) with numbers, titles, authors, branches, and timestamps. mode=get fetches one PR (by number) with body (truncated to ${MAX_PR_BODY_BYTES} bytes), merge state, and review summary; PRs with ≤100 commits include the 10 most recent inline. mode=files lists files changed by a PR with additions/deletions and optional per-file patches (truncated to ${MAX_PR_FILE_PATCH_BYTES} bytes each, max ${MAX_PR_FILES} files).`,
    inputSchema: PullsSchema,

    execute: async (input): Promise<AgentToolResult> => {
      if (input.mode === "get") return getPullRequest(deps, input)
      if (input.mode === "files") return listPrFiles(deps, input)
      return listPullRequests(deps, input)
    },
        ...
        JSON.stringify(
          input.mode === "list"
            ...
            : {
                ...
                repo: `${input.owner}/${input.repo}`,
                number: input.number,
                ...(input.mode === "files" ? { page: input.page } : {}),
              }
        ),
