 ...
 * - `load_attachment` and the section loaders — image vision and interactive
 *   paging are out of scope for a bounded text brief. `get_attachment` IS
 *   included so the researcher can pull an attachment's extracted text/data into
 *   the brief instead of citing a search snippet it cannot open.
 *
