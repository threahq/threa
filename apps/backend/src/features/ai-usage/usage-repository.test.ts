...
describe("AIUsageRepository.getUsageByDay", () => {
  ...
  it("buckets by day in the given timezone + function_id, scopes to the workspace/period, and maps numeric strings", async () => {
    ...
    const result = await AIUsageRepository.getUsageByDay(db, "ws_1", START, END, "Europe/Stockholm")

    expect(captured.text).toContain("FROM ai_usage_records")
    expect(captured.text).toContain("to_char(created_at AT TIME ZONE $")
    expect(captured.text).toContain("GROUP BY date, function_id")
    expect(captured.text).toContain("workspace_id =")
    expect(captured.values).toEqual(["Europe/Stockholm", "ws_1", START, END])
