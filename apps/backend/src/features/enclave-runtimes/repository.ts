 ...
 * per invocation, and the frontend encrypts each E2E message to every live
 * EIK plus the owner's UIK.
 */
...
export const EnclaveRuntimesRepository = {
   ...
   * dispatcher uses this to pick a target; the frontend uses this to build
   * the per-message recipient list. Sorted by `last_seen_at DESC` so the
   * freshest instance comes first (consumed by dispatcher round-robin /
   * random pick — order isn't load-bearing for correctness).
   */
