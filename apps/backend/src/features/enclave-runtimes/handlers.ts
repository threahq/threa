...
export function createEnclaveRuntimesHandlers({ enclaveRuntimesService }: Dependencies) {
  return {
    ...
    /**
     * GET /api/v1/workspaces/:workspaceId/enclave/active-keys
     * Frontend reads the current live EIK set to build per-message recipient
     * envelopes. Workspace-scoped at the route level; the data itself is
     * global, but workspace membership gates access.
     */
