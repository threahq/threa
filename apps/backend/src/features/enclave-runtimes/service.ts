 ...
 * before a stalled instance falls out of the recipient list and the
 * dispatcher's target pool.
 */
...
export class EnclaveRuntimesService {
  ...
  /**
   * Live EIK list. Used by:
   *  - the dispatcher to pick an instance to invoke,
   *  - the frontend (via `/enclave/active-keys`) to build per-message
   *    recipient envelopes.
   */
