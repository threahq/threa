...
export function createAttachmentHandlers({ attachmentService, streamService, storage, pool }: Dependencies) {
  return {
    ...
    async upload(req: Request, res: Response) {
      ...
      // E2E upload: the body bytes are client-side ciphertext. The server stores
      // them opaquely and forces placeholder metadata — the real filename/mime
      // ride encrypted in the message's attachmentRefs, never on this row.
      const e2e = req.body?.e2e === "true" || req.body?.e2e === true

      const uploadResult = await attachmentService.createForUpload({
        id: attachmentId,
        workspaceId,
        uploadedBy: userId,
        filename: e2e ? E2E_PLACEHOLDER_FILENAME : file.originalname,
