...
import { createBearerAuth, createInvokeHandler } from "./invoke-handler"
...
async function main() {
  ...
  const orchestrator = new Orchestrator(config, keyPair)
  const bearerAuth = createBearerAuth(config.internalApiKey)
  ...
  app.post("/invoke", bearerAuth, createInvokeHandler(orchestrator))
