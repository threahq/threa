...
import type { E2eRewrapNeededPayload } from "@threahq/types"
 ...
 * `e2e:rewrap_needed` at an E2E scratchpad owner when an agent there holds a
 * live key that no wrap addresses — a turn already parked for want of one, or a
 * bot runtime registering a key it has never held. Only the owner's unlocked
 * device can re-wrap: neither the enclave nor a bot can seal the stream key to
 * itself (INV-E7). The per-stream affordance only heals the stream the owner has
 * open; this lifts that to the workspace so a scratchpad the owner *isn't*
 * looking at still resolves the moment the signal lands, without waiting for
 * them to open it. A locked session can't re-wrap: this listener drops the
 * event, and the per-stream affordance only heals once the owner actually opens
 * that stream — so an owner who unlocks without visiting it relies on the
 * backend re-emitting the nudge (every `REWRAP_SOCKET_REEMIT_MS`) to a
 * now-unlocked tab. The graced web-push is the offline owner's path back.
 *
 * Side-effect only; renders nothing. Mounted once per workspace inside the
 * socket + E2E-session providers.
 */
export function RewrapNudgeListener({ workspaceId }: { workspaceId: string }): null {
  ...
  useEffect(() => {
    ...
    const handle = (payload: E2eRewrapNeededPayload) => {
    ...
    socket.on("e2e:rewrap_needed", handle)
    return () => {
      socket.off("e2e:rewrap_needed", handle)
    }
  }, [socket, workspaceId])
