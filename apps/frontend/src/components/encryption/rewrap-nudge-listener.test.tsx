...
import { RewrapNudgeListener } from "./rewrap-nudge-listener"

const WORKSPACE_ID = "ws_1"
const OWNER_ID = "usr_owner"
const PAYLOAD: E2eRewrapNeededPayload = {
...
function arrange(session: object, userId: string | undefined = OWNER_ID): FakeSocket {
  ...
  render(
    <QueryClientProvider client={queryClient}>
      <RewrapNudgeListener workspaceId={WORKSPACE_ID} />
    </QueryClientProvider>
  )
  return socket
}
...
describe("RewrapNudgeListener", () => {
  it("re-wraps the stuck stream's actor keys when an unlocked owner receives the nudge", async () => {
    ...
    socket.emit("e2e:rewrap_needed", PAYLOAD)
  ...
  it("ignores a nudge addressed to a different user (only the owner can re-wrap)", () => {
    ...
    socket.emit("e2e:rewrap_needed", { ...PAYLOAD, targetUserId: "usr_someone_else" })
  ...
  it("ignores a nudge for a different workspace", () => {
    ...
    socket.emit("e2e:rewrap_needed", { ...PAYLOAD, workspaceId: "ws_other" })
