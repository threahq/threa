 ...
 * The lookup derives the namespaced key from the pointer's reference pin centrally
 * and narrows to the shared-message slot type; a miss returns `null` so the view
 * renders the pre-hydration skeleton rather than crash.
 */
export function SlotsProvider({ map, children }: { map: SlotMap | undefined | null; children: ReactNode }) {
  const value = useMemo<SlotsContextValue>(() => {
    const snapshot = map ?? {}
    return {
      getSharedMessage: (messageId: string, version: number | null, range: ContentRange | null) => {
        const slot = snapshot[sharedMessageSlotKey(messageId, version, range)]
...
export function useSharedMessageSlot(
  messageId: string,
  version: number | null = null,
  range: ContentRange | null = null
): SharedMessageSlot | null {
  const ctx = useContext(SlotsCtx)
  if (!ctx) return null
  return ctx.getSharedMessage(messageId, version, range)
}
