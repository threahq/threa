...
describe("FollowUpScheduledEvent", () => {
  ...
  it("cancels via the API and flips the same button to Cancelled (INV-63: no success toast)", async () => {
    ...
    // Same element throughout (focus retained, aria-live announces) — the label
    // flips to "Cancelled" and it's aria-disabled rather than unmounted.
    const button = await screen.findByRole("button", { name: "Cancelled" })
    expect(button).toHaveAttribute("aria-disabled", "true")
    expect(screen.queryByRole("button", { name: "Cancel" })).not.toBeInTheDocument()
  })

  it("shows the authoritative Cancelled state when a matching cancelled patch is present", () => {
    render(
      <MemoryRouter>
