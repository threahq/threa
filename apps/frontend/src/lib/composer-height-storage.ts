 ...
 *
 * Why: the timeline's scroll viewport is inset above the floating composer
 * pill via `var(--composer-height, 0px)`. That variable is set by
 ...
 * render commits. Until then the fallback (0px) would win, so the first
 * timeline paint could size the scroll viewport underneath the composer before
 * correcting a frame later.
 *
 * Persisting the last-known height to `localStorage` and applying it to
 * `:root` before React mounts means the first render already reserves roughly
 * the right space. The editor-zone override that runs in the composer's layout
 * effect later corrects any residual drift pre-paint (and notifies the timeline
 * to re-anchor in the same frame), so there is no visible jump.
 */
