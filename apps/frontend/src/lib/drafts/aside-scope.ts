...
import { asideDraftScope } from "@threahq/types"

/**
 * Minting side of the aside draft scope. The grammar itself lives in
 * `@threahq/types` — the aside's agent reads these drafts server-side, so both
...
export { asideDraftScope, isAsideDraftScope, parseAsideDraftScope, type AsideDraftScope } from "@threahq/types"
