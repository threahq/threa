...
- Pill atoms (every inline atom above except `emoji`) reorder by drag. Mouse activation starts after 6px. On touch, a tap selects the whole pill for native copy; moving that selected pill by more than 10px starts a drag. An unselected pill leaves pre-hold movement to scrolling, then claims the shared 500ms hold so first-attempt hold-and-move remains reachable; releasing that hold without movement selects the pill instead. A stationary hold on an already-selected pill remains available to the OS copy menu. The source greys in place while a gold caret snaps between words; touch mirrors that slot in a contextual loupe above the thumb and pulses haptics as the target changes. Release commits one undoable move.
