...
| GET    | `/workspaces/{ws}/delegations`                    | `delegations:read`  | List open delegated tasks (oldest first). The discovery surface for a local agent — each carries `brief` + `contextRefs`.                                                                                           |
| POST   | `/workspaces/{ws}/delegations/{id}/claim`         | `delegations:write` | Claim an open task. Body `{claimedByLabel}`. Returns `{…, claimToken, claimExpiresAt}` — **the token is shown once**. 409 if not open.                                                                              |
| POST   | `/workspaces/{ws}/delegations/{id}/heartbeat`     | `delegations:write` | Push the claim expiry forward. Body `{claimToken}`. Returns `{id, status, claimExpiresAt}`.                                                                                                                         |
| POST   | `/workspaces/{ws}/delegations/{id}/status`        | `delegations:write` | Report progress → `running`. Body `{claimToken, statusNote?}`. Renews the claim TTL.                                                                                                                                |
| POST   | `/workspaces/{ws}/delegations/{id}/complete`      | `delegations:write` | Complete with a result. **201**. Body `{claimToken, resultMarkdown, sources?, metadata?}` — `resultMarkdown` posts as a message into the stream (authored by the key's principal); returns `{delegation, message}`. |
| POST   | `/workspaces/{ws}/delegations/{id}/fail`          | `delegations:write` | Fail the task. Body `{claimToken, errorMessage}`.                                                                                                                                                                   |
