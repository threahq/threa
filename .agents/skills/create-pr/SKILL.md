...
3. **Self-review + verify every claim.** If `.tmp/build-feature-reviewed.diff` and `.tmp/build-feature-review-evidence.md` exist, read the recorded `base_ref`, generate the current patch with the same `{ git diff "$base_ref"...HEAD; git diff HEAD; }` shape, and compare it byte-for-byte. A match means the broad adversarial review plus targeted post-fix recheck covers the final diff: verify PR prose and evidence only; do not repeat the invariant audit or `/code-review`. Without matching evidence, walk the diff for race-safe writes (INV-20), workspace scoping (INV-8), outbox-in-tx (INV-4/7), canonical access predicates (INV-62), no silent fallbacks (INV-11), Zod inputs (INV-55), and dead code (INV-38); substantial changes then run `/code-review`. In both paths, verify each draft claim literally. Findings → fix in branch and report them honestly.
